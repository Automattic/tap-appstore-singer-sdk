...
class AppStoreStream(Stream):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.float_fields = {
            ...
            if "number" in prop["type"]
        }
        self.int_fields = {
            name
            for name, prop in self.schema["properties"].items()
            if "integer" in prop["type"]
        }
    ...
    def get_start_date(self, context: dict = None):
        ...
        return (
            starting_timestamp + self.date_increment
    ...
    def get_records(self, context: dict = None):
        ...
        while report := self._get_report(start_date):
            ...
            logger.info(
                f"Extracting {self.tap_stream_id} starting from {start_date_fmt}"
            )
            ...
            for record in reader:
                ...
                if first_value and any(
                    keyword in first_value for keyword in self.skip_line_first_values
                ):
                    logger.info(
                        f"Skipping line report {start_date_fmt}: {record.values()}"
                    )
                    continue

                line_id += 1
                record["_line_id"] = line_id
                record["_time_extracted"] = datetime.utcnow().strftime(
                    "%Y-%m-%dT%H:%M:%SZ"
                )
    ...
    def _get_report(self, start_date):
        try:
            ...
            if (
                str(e).startswith("There were no")
                and str(e).endswith("for the date specified.")
                or str(e).startswith("Report is not available yet")
            ):
                logger.info(str(e))
                return None
            raise
    ...
    def convert_date(date_str, date_format="%Y-%m-%d"):
        ...
        if not date_str or date_str.strip() == "":
            ...
            logger.warning(
                f"Empty or invalid date string provided; cannot convert using format {date_format}"
            )
            return None
        ...
        try:
            ...
            logger.error(
                f"Date conversion error for date: '{date_str}' with format '{date_format}' | Error: {str(e)}"
            )
            raise
